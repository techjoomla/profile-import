...
class LinkedInAPI {
  ...
  protected $response_format         = self::_DEFAULT_RESPONSE_FORMAT;
    ...
    if(!is_array($config)) {
	  ...
	  //echo'<br/>CLASS in class constructor<br/>';print_r($this);
	}
  ...
  /**
    ...
    if(!is_string($token) || !is_string($secret) || !is_string($verifier)) {
      	// nothing passed, raise an exception
		  	...
		  	$return_data=array();
        if(JRequest::getVar('oauth_problem')=='user_refused')
        {
        	$return_data['error']   = 'user_refused';
       		$return_data['success'] = FALSE;
        }
      	else{
      		$return_data['error']   = 'bad data passed, string type is required for $token, $secret and $verifier';
       		$return_data['success'] = FALSE;
      		return $return_data;
      	}
    }
